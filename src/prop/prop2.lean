...
notation `[`v`]` ` ⊧ ` φ :25 := satisfy v φ
-- infix ` ⊧ ` :25 := models

def valid (φ) := ∀ v, [v] ⊧ φ
prefix `⊧` :25 := valid

lemma valid_P1 (φ ψ) : ⊧ (φ →' ψ →' φ) := by tauto
